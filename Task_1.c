...
void filter_ascending(char* string_1)
{
    ...
    {
        for (int j = 0; string_1[j+1] != '\0'; j++) //not compare last two characters
        {
            if (string_1[j] > string_1[j+1])
            {
                for (int k = j+1; string_1[k] != '\0'; k++) //was overwriting string from first character
                {
